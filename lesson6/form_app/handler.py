...
def admin_edit_form(request: Request, form_id: int) -> Response:
    ...
    # >>> КЛЮЧЕВОЕ: передаём form_id в шаблон
...
def admin_edit_form_post(request: Request, content: dict, form_id: int) -> Response:
    ...
    try:
        ...
        # собираем ошибки в куки
        for err in e.errors():
            loc, msg = err["loc"][0], err["msg"]
            ...
            cookies[f"{loc}_err"] = quote(msg.capitalize())
        ...
        for field in UserFormModel.model_fields:
            ...
            if isinstance(val, list):
                val = "|".join(val)
            cookies[field] = quote(val)
            
        # редирект обратно на форму с тем же form_id
        return Response(303, {"Location": f"/admin/edit/{form_id}"}, cookies, "")

    # если всё валидно — сохраняем и уходим на дашборд
    update_user_form_by_id(form_id, form_data)
    return Response(303, {"Location": "/admin/dashboard"}, cookies, "")
